...
function AffaireFilter({ affaires, selected, onChange }) {
  ...
  const label = selected.length === 0
    ...
    : selected.length === 1
      ...
      : `${selected.length} affaires sélectionnées`
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <span>{label}</span>
        {selected.length > 0
          ? <span onClick={e => { e.stopPropagation(); onChange([]) }} className="ml-1 text-blue-400 hover:text-blue-700 font-bold">✕</span>
          : <span className="text-slate-400">{open ? '▲' : '▼'}</span>
        }
      </button>

      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-slate-200 rounded-xl shadow-xl w-80 flex flex-col" style={{ maxHeight: '70vh' }}>
          ...
          <div className="px-3 pt-3 pb-2 border-b border-slate-100 shrink-0">
            <input
              ...
              placeholder="Rechercher par n°, intitulé, client…"
            ...
            <div className="flex items-center justify-between mt-1.5">
              <span className="text-xs text-slate-400">{visible.length} affaire{visible.length !== 1 ? 's' : ''}</span>
              {selected.length > 0 && (
            ...
            {visible.map(a => {
              ...
              return (
                <button key={a.id} onClick={() => toggle(a.id)}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 hover:bg-slate-50 text-left transition-colors ${checked ? 'bg-blue-50' : ''}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {a.client && <div className="text-slate-400 text-xs truncate">{a.client}</div>}
                  </div>
                </button>
              )
            })}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="shrink-0 flex items-center justify-between gap-4 px-4 py-2 bg-white border-b border-slate-200">
        ...
        {/* Filtre affaire avec recherche */}
        <AffaireFilter
          affaires={affaires.filter(a => !effectiveCA || a.caId === effectiveCA)}
